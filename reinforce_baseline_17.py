...
class crane_env():
    ...
    def step(self, F, t, x_star, pos_done_count, ang_done_count):            
        ...
        theta_pen = -softplus(torch.tensor((Z_new[0]*100)**2 - (theta_buffer*100)**2).float()).numpy()
...
class Policy(nn.Module):
    def __init__(self):
        ...
        self.affine1_p = nn.Linear(6, 64)
        self.affine2_p = nn.Linear(64,81)
        #self.affine3_p = nn.Linear(41, 41)
        
        self.affine1_v = nn.Linear(6, 64)
...
SAVE_PATH = 'crane_ideal_coarse'+ run_num + '.pth'
...
np.save('crane_ideal_fine_reward_' + run_num + '.npy', reward_graph)
'''
np.save('crane_ideal_coarse_time_' + run_num + '.npy', task_time_graph)