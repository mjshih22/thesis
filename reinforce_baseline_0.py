...
class Policy(nn.Module):
    def __init__(self):
        ...
        self.affine1_v = nn.Linear(2, 128)
        self.affine2_v = nn.Linear(128, 1)
    ...
    def forward(self, inputs):
        ...
        x = F.relu(self.affine1_v(inputs))
        state_value = self.affine2_v(x)
...
SAVE_PATH = 'rebase0_'+ run_num + '.pth'
...
for i_episode in range(epochs):   
    ...
    if i_episode % log_interval == 0 and i_episode != 0:
        print('Episode {} Last length {} Average length: {:.2f} Last reward: {:.2f} Average reward: {:.2f}  \n Last p_loss: {:.2f} Last v_loss: {:.2f}'.format(
            i_episode, t, task_time, last_reward/60, avg_reward/60, p_loss, v_loss))
...
np.save('v_loss_graph0_' + run_num + '.npy', v_loss_graph)
